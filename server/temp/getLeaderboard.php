...
$sql = $sql. "(select (select ROUND(sum(value), 2) from transactions t2 where t2.id <= t1.id and t2.userId = t1.userId) as profitATH from transactions t1 where t1.userId = crashgamebets.userId ORDER BY profitATH DESC LIMIT 1) as profitATH, ";
$sql = $sql. "(select (select ROUND(sum(value),2) from transactions t2 where t2.id <= t1.id and t2.userId = t1.userId) as profitATL from transactions t1 where t1.userId = crashgamebets.userId ORDER BY profitATL ASC LIMIT 1) as profitATL, ";
